...
The `unit-tests.yml` workflow covers Python scripts in two directories:
...
## Coverage baseline (measured 2026-06-10, 83 tests)
...
entrypoints in each script — those paths parse `sys.argv` and write to disk/stdout, so they need
`subprocess` or `monkeypatch` to cover.
...
When a new Python script is added to `bootc-build/create-release/scripts/` or `scripts/`:
