...
| Action | Status | Notes |
|---|---|---|
| `setup-runner` | **Adopted** | `update-podman: true`, `native-overlay: true`, `storage-backend: btrfs`, `install-tools: '["just"]'`; Podman 5 plus native rootful overlay is required by Dakota's nested chunkify mount path |
